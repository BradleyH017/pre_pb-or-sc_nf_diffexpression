...
optionList <- list(
  ...
  optparse::make_option(c("-t", "--bulk_count_threshold"),
                        type = "double",
                        default = 10,
                        help = "Minimum raw count for a gene to be
                        considered detected in a sample."
  ),

  optparse::make_option(c("-u", "--sample_frac_threshold"),
                        type = "double",
                        default = 0.1,
                        help = "Minimum proportion of samples in which a
                        gene must have counts > bulk_count_threshold to be
                        retained."
  ),
...
bulk_count_threshold <- arguments$options$bulk_count_threshold
sample_frac_threshold <- arguments$options$sample_frac_threshold
...
if (arguments$options$pre_filter_genes) {
  ...
  print(sprintf(paste("Filtering genes without counts > %s in at least %s",
                      "proportion of samples BEFORE finding differentially",
                      "expressed genes."),
                bulk_count_threshold, sample_frac_threshold))
  n_genes_pre_bulk_filter <- nrow(counts_matrix)

  # Filter and update all matrices
  frac_samples_expressed <- Matrix::rowMeans(
    counts_matrix > bulk_count_threshold
  )
  counts_matrix <- counts_matrix[
    frac_samples_expressed >= sample_frac_threshold,
  ]
  cp10_matrix <- cp10_matrix[rownames(counts_matrix), ]
  logcp10_matrix <- logcp10_matrix[rownames(counts_matrix), ]

  print(
    sprintf("Retaining %s / %s genes.",
            nrow(counts_matrix), n_genes_pre_bulk_filter)
  )
}
